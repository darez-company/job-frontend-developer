...
test.describe('Chat with Scripted Options Flow', () => {
    ...
    test('should allow a user to click an option, see their message, a typing indicator, and then the bot reply', async ({ page }) => {
        // Arrange
        const firstStep = conversationSteps[0];
        const firstOptionText = firstStep.options![0];
        const secondOptionText = firstStep.options![1];

        const firstOptionButton = page.getByRole('button', { name: firstOptionText });
        const secondOptionButton = page.getByRole('button', { name: secondOptionText });
    ...
    test('should allow a user to type a message and receive an AI response', async ({ page }) => {
        // Arrange
